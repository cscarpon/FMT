...
The following function `pc_obj$methods()` shows the methods of th pc_obj class, which are inherited functions tof that object
...
to call a method use `pc_obj$method_name()`

initialize(pc_obj, or .rdata),  
save_chm(pc_obj$CHM, filepath) saves the currently stored CHM to the filepath,
...
to initialise the object use `pc_obj$new(filepath)`
